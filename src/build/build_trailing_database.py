...
master_boxscore = pd.DataFrame(read_database("./src/sql/database.db", f"boxscore {seasons[0]}"))

for season in seasons[1:]:
    master_boxscore = pd.concat([master_boxscore,read_database("./src/sql/database.db", f"boxscore {season}")], axis=0)
...
print(f"Building {n} game trailing database...")
...
update_database("./src/sql/trailing_database.db", f"{n}_game_trailing",trailing_dataset)